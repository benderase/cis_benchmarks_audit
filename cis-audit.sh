...
##
## Edited and modified For Red Hat Enterprise Linux 7 CIS Benchmark V3.0.0 - 06-25-2020 By Haim Asher
...
test_1.2.4() {
    ...
    ## Tests Start ##
    [ $(timeout 30 subscription-manager identity | wc -l) -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_1.2.5() {
    ...
    ## Tests Start ##
          [ $(systemctl is-enabled rhnsd) == "disabled" ] && result="Pass"
    fi
    ## Tests End ##
...
test_1.3.2() {
    ...
    ## Tests Start ##
    state=1
    [ $(grep -Ei '^\s*Defaults\s+([^#]\S+,\s*)?use_pty\b' /etc/sudoers /etc/sudoers.d/* | wc -l) -ne 0 ] && state=0
    [ $state -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_1.3.3() {
    ...
    ## Tests Start ##
    state=1
    [ $(grep -Ei '^\s*Defaults\s+([^#;]+,\s*)?logfile\s*=\s*(")?[^#;]+(")?' /etc/sudoers /etc/sudoers.d/* | wc -l) -ne 0 ] && state=0
    [ $state -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_1.7.1.4() {
    id=$1
    level=$2
    description="Ensure the SELinux mode is enforcing or permissive"
    scored="Scored"
    test_start_time=$(test_start $id)
    ...
    [[ "$(grep -Ei '^\s*SELINUX=(enforcing|permissive)' /etc/selinux/config)" == @(SELINUX=enforcing|SELINUX=permissive) ]] || state=1
    [[ "$(getenforce)" == @(enforcing|permissive) ]] || state=1
    [ $state -eq 0 ] && result="Pass"
    ## Tests End ##
...
if [ $(is_test_included 1; echo $?) -eq 0 ]; then   write_cache "1,Initial Setup"
    ...
    if [ $(is_test_included 1.1; echo $?) -eq 0 ]; then   write_cache "1.1,Filesystem Configuration"
        ...
        if [ $(is_test_included 1.1.1; echo $?) -eq 0 ]; then   write_cache "1.1.1,Disable unused filesystems"
            ...
            run_test 1.1.1.2 1 test_1.1.1.x squashfs    ## Ensure mounting of squashfs is disabled
            run_test 1.1.1.3 1 test_1.1.1.x udf   ## Ensure mounting of udf is disabled
            run_test 1.1.1.4 1 test_1.1.1.x fat   ## Ensure mounting of fat is disabled
            run_test 1.1.1.4 1 test_1.1.1.x vfat   ## Ensure mounting of vfat is disabled
            run_test 1.1.1.4 1 test_1.1.1.x mdos   ## Ensure mounting of mdos is disabled
        fi
        run_test 1.1.2 2 test_1.1.x-check_partition /tmp   ## 1.1.2 Ensure separate partition exists for /tmp
        run_test 1.1.3 1 test_1.1.x-check_fs_opts /tmp noexec   ## 1.1.3 Ensure noexec option set on /tmp
        run_test 1.1.4 1 test_1.1.x-check_fs_opts /tmp nodev   ## 1.1.4 Ensure nodev option set on /tmp
        run_test 1.1.5 1 test_1.1.x-check_fs_opts /tmp nosuid   ## 1.1.5 Ensure nosuid option set on /tmp
        run_test 1.1.6 2 test_1.1.x-check_partition /dev/shm   ## 1.1.6 Ensure separate partition exists for /dev/shm
        run_test 1.1.7 1 test_1.1.x-check_fs_opts /dev/shm noexec   ## 1.1.7 Ensure noexec option set on /dev/shm
        run_test 1.1.8 1 test_1.1.x-check_fs_opts /dev/shm nodev   ## 1.1.8 Ensure nodev option set on /dev/shm
        run_test 1.1.9 1 test_1.1.x-check_fs_opts /dev/shm nosuid   ## 1.1.9 Ensure nosuid option set on /dev/shm
        run_test 1.1.10 2 test_1.1.x-check_partition /var   ## 1.1.10 Ensure separate partition exists for /var
        run_test 1.1.11 2 test_1.1.x-check_partition /var/tmp   ## 1.1.11 Ensure separate partition exists for /var/tmp
        run_test 1.1.12 1 test_1.1.x-check_fs_opts /var/tmp noexec   ## 1.1.12 Ensure noexec option set on /var/tmp
        run_test 1.1.13 1 test_1.1.x-check_fs_opts /var/tmp nodev   ## 1.1.13 Ensure nodev option set on /var/tmp
        run_test 1.1.14 1 test_1.1.x-check_fs_opts /var/tmp nosuid   ## 1.1.14 Ensure nosuid option set on /var/tmp
        run_test 1.1.15 2 test_1.1.x-check_partition /var/log   ## 1.1.15 Ensure separate partition exists for /var/log
        run_test 1.1.16 2 test_1.1.x-check_partition /var/log/audit   ## 1.1.16 Ensure separate partition exists for /var/log/audit
        run_test 1.1.17 2 test_1.1.x-check_partition /home   ## 1.1.17 Ensure separate partition exists for /home
        run_test 1.1.18 1 test_1.1.x-check_fs_opts /home nodev   ## 1.1.18 Ensure nodev option set on /home
        run_test 1.1.19 1 test_1.1.x-check_removable noexec  ## 1.1.19 Ensure noexec option set on removable media partitions
        run_test 1.1.20 1 test_1.1.x-check_removable nodev  ## 1.1.20 Ensure nodev option set on removable media partitions
        run_test 1.1.21 1 test_1.1.x-check_removable nosuid  ## 1.1.21 Ensure nosuid option set on removable media partitions
        ...
        run_test 1.1.24 1 test_1.1.1.x usb-storage   ## Ensure mounting of mdos is disabled
    fi

    ## Section 1.2 - Configure Software Updates
    if [ $(is_test_included 1.2; echo $?) -eq 0 ]; then   write_cache "1.2,Configure Software Updates"
        run_test 1.2.1 1 test_1.2.1   ## 1.2.1 Ensure GPG keys are configured
        #run_test 1.2.1 1 skip_test "Ensure package manager repositories are configured"
        run_test 1.2.2 1 test_1.2.2   ## 1.2.2 Ensure package manager repositories are configured
        run_test 1.2.3 1 test_1.2.3   ## 1.2.3 Ensure gpgcheck is globally activated
        run_test 1.2.4 1 test_1.2.4   ## 1.2.4 Ensure Red Hat Subscription Manager connection is configured
        run_test 1.2.5 1 test_1.2.5   ## 1.2.5 Disable the rhnsd Daemon
    fi

    ## Section 1.3 - Configure sudo
    if [ $(is_test_included 1.3; echo $?) -eq 0 ]; then   write_cache "1.3,Configure sudo"
        run_test 1.3.1 1 test_is_installed sudo SUDO   ## 1.3.1 Ensure SUDO is installed
        run_test 1.3.2 1 test_1.3.2   ## 1.3.2 Ensure sudo commands use pty
        run_test 1.3.3 1 test_1.3.3   ## 1.3.3 Ensure sudo log file exists
    fi

    ## Section 1.4 - Filesystem Integrity Checking
    if [ $(is_test_included 1.3; echo $?) -eq 0 ]; then   write_cache "1.4,Filesystem Integrity Checking"
        run_test 1.4.1 1 test_is_installed aide AIDE   ## 1.4.1 Ensure AIDE is installed
        run_test 1.4.2 1 test_1.4.2   ## 1.4.2 Ensure filesystem integrity is regularly checked
    fi

    ## Section 1.5 - Secure Boot Settings
    if [ $(is_test_included 1.5; echo $?) -eq 0 ]; then   write_cache "1.5,Secure Boot Settings"
        run_test 1.5.1 1 test_1.5.1   ## 1.5.1 Ensure bootloader password is set
        run_test 1.5.2 1 test_perms 600 /boot/grub2/grub.cfg   ## 1.5.2 Ensure permissions on bootloader config are configured
        run_test 1.5.2 1 test_perms 600 /boot/grub2/user.cfg   ## 1.5.2 Ensure permissions on bootloader config are configured
        run_test 1.5.3 1 test_1.5.3   ## 1.5.3 Ensure authentication requires for single user mode

    fi

    ## Section 1.6 - Additional Process Hardening
    if [ $(is_test_included 1.6; echo $?) -eq 0 ]; then   write_cache "1.6,Additional Process Hardening"
        run_test 1.6.1 1 test_1.6.1   ## 1.6.1 Ensure core dumps are restricted
        run_test 1.6.2 1 test_1.6.2   ## 1.6.2 Ensure XD/NX support is enabled
        run_test 1.6.3 1 test_1.6.3   ## 1.6.3 Ensure address space layout randomisation (ASLR) is enabled
        run_test 1.6.4 1 test_1.6.4   ## 1.6.4 Ensure prelink is disabled

    fi

    ## Section 1.7 - Mandatory Access Control
    if [ $(is_test_included 1.7; echo $?) -eq 0 ]; then   write_cache "1.7,Mandatory Access Control"
        run_test 1.7.1.1 1 test_is_installed libselinux SELinux   ## 1.7.1.1 Ensure SELinux is installed
        if [ $(is_test_included 1.7.1; echo $?) -eq 0 ]; then   write_cache "1.7.1,Configure SELinux"
            run_test 1.7.1.2 2 test_1.7.1.2   ## 1.7.1.2 Ensure SELinux is not disabled in bootloader configuration
            run_test 1.7.1.3 2 test_1.6.1.3   ## 1.7.1.3 Ensure SELinux policy is configured
            run_test 1.7.1.4 2 test_1.7.1.4   ## 1.7.1.4 Ensure the SELinux mode is enforcing or permissive
            run_test 1.7.1.5 2 test_1.7.1.5   ## 1.7.1.5 Ensure the SELinux mode is enforcing
            run_test 1.7.1.6 2 test_1.7.1.6   ## 1.7.1.6 Ensure no unconfined services exist
            run_test 1.7.1.7 2 test_1.7.1.7   ## 1.7.1.7 Ensure SETroubleshoot is not installed
            run_test 1.7.1.8 2 test_1.7.1.8   ## 1.7.1.8 Ensure MCS Translation Service (mcstrans) is not installed
